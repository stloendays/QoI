...
assert abs(float(resolved[(0.10,'zfp_vs_sz3')]['effect']) - 0.5574478963397762) < 1e-12
assert abs(float(resolved[(0.10,'zfp_vs_sperr')]['effect']) - 0.6007474002367663) < 1e-12
assert abs(float(resolved[(0.10,'sz3_vs_sperr')]['effect']) - 1.0325070260736608) < 1e-12
assert (int(d[(0.10,'zfp_vs_sz3')]['n_pairs']), int(d[(0.10,'zfp_vs_sz3')]['n_materials'])) == (457, 214)
assert (int(d[(0.10,'zfp_vs_sperr')]['n_pairs']), int(d[(0.10,'zfp_vs_sperr')]['n_materials'])) == (465, 206)
assert (int(d[(0.10,'sz3_vs_sperr')]['n_pairs']), int(d[(0.10,'sz3_vs_sperr')]['n_materials'])) == (1848, 254)
...
for c in calipers:
    for p, label in pair_label.items():
        ...
        machine.append({
            ...
            'pair': p,
            'pair_label': label,
            'n_pairs': int(dg['n_pairs']),
            'n_materials': int(dg['n_materials']),
            'median_abs_log10_linf_difference_dex': float(dg['median_abs_log10_Linf_difference_dex']),
            'p95_abs_log10_linf_difference_dex': float(dg['p95_abs_log10_Linf_difference_dex']),
            'median_linf_ratio_larger_over_smaller': float(dg['median_larger_over_smaller_realized_Linf']),
            'p95_linf_ratio_larger_over_smaller': float(dg['p95_larger_over_smaller_realized_Linf']),
            'resolved_bader_ratio': float(rr['effect']),
            'resolved_ci_low': float(rr['ci_low']),
            'resolved_ci_high': float(rr['ci_high']),
            'resolved_n_usable_pairs': int(rr['n_usable_pairs']),
            'resolved_n_usable_materials': int(rr['n_usable_materials']),
            'fixed_bader_ratio': float(ff['effect']),
            'fixed_ci_low': float(ff['ci_low']),
            'fixed_ci_high': float(ff['ci_high']),
        })
write_csv(machine)
...
for r in machine:
    lines.append(f"| {r['caliper_dex']:.2f} | {r['pair_label']} | {r['n_pairs']:,} | {r['n_materials']} | {r['median_abs_log10_linf_difference_dex']:.4f} | {r['p95_abs_log10_linf_difference_dex']:.4f} | {r['median_linf_ratio_larger_over_smaller']:.3f} | {r['p95_linf_ratio_larger_over_smaller']:.3f} |")
lines += [
