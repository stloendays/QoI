...
for r in main:
    ...
    # This is the metric that corresponds to fixed-basin underestimation in the
    # main Figure 5 comparison: maximum re-derived error / maximum fixed-basin error.
...
# Baseline charge differences relative to BaderKit on-grid.
baseline = {}
for mid, meta in panel_map.items():
    if meta.get('sentinel'):
        continue
...
# Paired codec responses at the two mechanism-study relative tolerances.
pairs = []
for mid, meta in panel_map.items():
    if meta.get('sentinel'):
        continue
    for codec in ('zfp', 'sz3', 'sperr'):
        for rel in ('0.0001', '0.001'):
            ...
            if not a or a.get('status') != 'SUCCESS':
                continue
            for solver in ('henkelman_ongrid', 'henkelman_neargrid'):
...
for solver in ('henkelman_ongrid', 'henkelman_neargrid'):
    ...
    for filter_name, pred in filters:
        ...
        impl_summary.append({
            'metric': 'codec_response_ratio', 'solver': solver, 'filter': filter_name,
            'n': len(vals), 'median': median(vals), 'q25': quantile(vals, .25), 'q75': quantile(vals, .75)
        })

# Noise-floor agreement and eligibility concordance.
...
for solver in ('henkelman_ongrid', 'henkelman_neargrid'):
    ratios = []
    print_agree = 0
    concord = {1e-4: 0, 1e-3: 0, 1e-2: 0}
    nmat = 0
    for mid, meta in panel_map.items():
        if meta.get('sentinel'):
            continue
        a = stab_map[(mid, 'baderkit_ongrid')]
        b = stab_map[(mid, solver)]
        ...
        nmat += 1
    impl_summary.append({
        'metric': 'noise_floor_ratio', 'solver': solver, 'filter': 'all_12_representatives',
        'n': len(ratios), 'median': median(ratios), 'q25': quantile(ratios, .25), 'q75': quantile(ratios, .75)
    })
    impl_summary.append({
        'metric': 'noise_floor_print_agreement', 'solver': solver, 'filter': 'abs_difference_le_1e-6',
        'n': nmat, 'median': print_agree / nmat, 'q25': '', 'q75': ''
    })
    for tau in (1e-4, 1e-3, 1e-2):
        impl_summary.append({
            'metric': f'eligibility_concordance_{tau:g}', 'solver': solver, 'filter': 'all_12_representatives',
            'n': nmat, 'median': concord[tau] / nmat, 'q25': '', 'q75': ''
        })
...
assert abs(float(ongrid_pair['median']) - 1.0) < 0.01
assert 0.7 < float(near_pair['median']) < 0.85
ongrid_print = next(r for r in impl_summary if r['metric']=='noise_floor_print_agreement' and r['solver']=='henkelman_ongrid')
assert round(float(ongrid_print['median']) * 12) == 10

# Strict 1e-4 codec ordering by implementation.
order_rows = []
for solver in ('baderkit_ongrid', 'henkelman_ongrid', 'henkelman_neargrid'):
    for numerator in ('sz3', 'sperr'):
        ...
        for mid, meta in panel_map.items():
            ...
            if not z or not n or z.get('status') != 'SUCCESS' or n.get('status') != 'SUCCESS':
                continue
            ze = float(z['max_charge_error_e']); ne = float(n['max_charge_error_e'])
            if ze > 0 and ne > 0:
                vals.append(ne / ze)
        order_rows.append({
            'solver': solver, 'ratio': f'{numerator.upper()}/ZFP', 'n': len(vals),
            'median_ratio': median(vals), 'q25': quantile(vals, .25), 'q75': quantile(vals, .75)
        })
write_csv(SUP / 'S11_codec_order_1e4.csv', order_rows)

# Exact BaderKit decomposition by perturbation kind.
...
for kind in sorted({r['kind'] for r in dec}):
    ...
    for r in rr:
        ...
        if d + i > 0:
            shares.append(d / (d + i))
        dominates.append(d > i)
        closure.append(float(r['closure_max_e']))
    dec_summary.append({
        'kind': kind, 'n': len(rr), 'median_domain_abs_share': median(shares),
        'q25': quantile(shares, .25), 'q75': quantile(shares, .75),
        'fraction_domain_gt_integrand': sum(dominates) / len(dominates),
        'max_closure_e': max(closure)
    })
...
for solver in ('baderkit_ongrid', 'henkelman_ongrid', 'henkelman_neargrid'):
    for control in ('global', 'shift', 'stratified'):
        ...
        sp_summary.append({
            'solver': solver, 'control': control, 'n': len(logs),
            'median_log2_control_over_codec': median(logs),
            'q25': quantile(logs, .25), 'q75': quantile(logs, .75),
            'fraction_increased': sum(float(r['difference_e']) > 0 for r in rr) / len(rr)
        })
...
lines.extend([
    ...
    '| Quantity | Result |', '|---|---:|',
    '| Expected outcome rows | 1,560 |', '| Completed outcome rows | 1,560 |',
    '| BaderKit on-grid solver failures | 2 |', '| Henkelman on-grid solver failures | 0 |', '| Henkelman near-grid solver failures | 0 |',
    '| Representative systems in aggregate | 12 development systems |', '| Separate probe-failure sentinel | 1 (`mp-1007755`; excluded from aggregates) |', '',
...
lines.extend(['', '### S11d. Exact BaderKit decomposition by perturbation kind', '', '| Perturbation kind | n | Median bounded domain share | IQR | Domain > integrand | Max closure residual (e) |', '|---|---:|---:|---:|---:|---:|'])
...
lines.extend(['', '### S11e. Spatial reorganization controls', '', '| Solver | Control | n pairs | Median log2(control/codec) | IQR | Fraction increased |', '|---|---|---:|---:|---:|---:|'])
for r in sp_summary:
...
for r in impl_summary:
    if r['metric'] in {'codec_response_ratio', 'noise_floor_print_agreement'}:
        print('impl', r)
for r in order_rows:
    print('order', r)
for r in dec_summary:
