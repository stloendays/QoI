...
**Reader-facing terminology:** use **QoI Stability Qualification (QSQ)**, **QSQ stability floor**, **QSQ eligibility**, and **archived float32 probe**. Historical identifiers such as `Protocol A.1`, `_A1`, and frozen protocol filenames are retained only when referring to repository provenance.
...
6. Reader-facing cross-references must not reintroduce internal version labels as scientific method names.
...
| Results — first definition of QSQ | five fixed-seed perturbations and material-specific stability floor | **Supplementary Table S2; Supplementary Fig. S2** | qualification definition, seed and amplitude validation |
...
| Results — qualification-probe correction | archived float32 probe is order-preserving; QSQ perturbation probe is required | **Supplementary Fig. S2; Supplementary Table S7** | calibration, seed spread, amplitude dependence |
...
| Methods — QSQ | exact seeds, floor definition and archived float32 predecessor | **Supplementary Table S2** | qualification reproducibility |
...
These are compact insertion forms for final assembly.
...
After QSQ is first defined:

> The archived float32-probe comparison, five-seed calibration and amplitude sensitivity are detailed in Supplementary Tables S2 and S7 and Supplementary Fig. S2.
...
- [ ] Reader-facing text uses `QSQ`, not an internal protocol version label.
- [ ] The archived float32 predecessor is clearly labelled archived/provenance-only.
