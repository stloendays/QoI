...
Electronic-structure calculations routinely generate high-dimensional floating-point data whose storage, transfer and repeated analysis can become a substantial computational burden. Error-bounded lossy compression has consequently become an important data-reduction strategy in scientific computing, with mature general-purpose compressors including ZFP, SZ3 and SPERR [1–4]. More broadly, lossy compression has been deployed to reduce storage and I/O costs across scientific workflows [18], including quantum-chemistry data such as two-electron integrals and application-specific GAMESS data streams [3,21]. Recent electronic-structure work has also explored lossy reduction inside density-functional calculations, although in that case the compressed object is the atomic-orbital basis rather than a stored real-space electron-density field [19].

The central difficulty is that a field-level error bound does not automatically define the fidelity of every scientific quantity subsequently extracted from the reconstruction. Z-checker formalized systematic post-compression assessment of reconstructed scientific data [5]. Mathematical and algorithmic work has since moved explicitly toward derived quantities: Jiao *et al.* developed error-control theory for several QoI families [6], and multilevel methods have provided quantitative control of user-prescribed derived quantities during data reduction [12]. Application-specific pipelines have further preserved or constrained QoIs in large scientific workflows, including fusion data [13,14,20]. In parallel, topology-aware compressors have shown that strict pointwise bounds need not preserve discrete topological structure and have introduced explicit topological guarantees [7,15]. Most recently, TOPIQ has addressed statistical prediction of QoI-level bias and uncertainty from compressed data [11]. Together, these studies establish that downstream fidelity must be treated as a first-class scientific requirement. They leave a logically prior question unresolved: **when is the requested QoI tolerance itself a valid benchmark?**
...
The chemistry context also matters for scope. Lossy compression is already used or proposed inside quantum-chemistry and electronic-structure workflows [3,18,19,21], but those studies address different compressed objects and validation targets. PaSTRI compresses two-electron integrals [21], SZ3 includes a customized GAMESS data-compression use case [3], and recent DFT work compresses an atom-centred orbital representation [19]. None of these results removes the need to validate a downstream real-space density analysis such as Bader partitioning. Our contribution is therefore not a claim that electronic-structure data have never been compressed; it is a claim about how downstream scientific validity should be assigned when a compressed field is later subjected to a numerically sensitive analysis.
...
The canonical numbered references are maintained in `paper/REFERENCES.md`. This draft uses references [1–21] from that file.