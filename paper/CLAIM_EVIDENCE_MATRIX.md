...
| 7 | A QoI must be stability-qualified before it can serve as a fidelity contract | **QSQ**: 41% of all 319 systems have no Bader charge defined to 1e-3 e; 80% not to 1e-4 e; and the qualification is not predictable by inspection [archived float32 probe: 7.2% / 23.2%] | `protocol/PROTOCOL_A1.md`, `stability/eligibility_by_threshold_A1.csv` | F3, F4 | **CONFIRMED** | Claim 14 now gives the direct consequence for binary benchmark labels |
...
| 14 | Strict-threshold binary Bader benchmarking systematically misattributes non-evaluable decisions as codec failures | Development corpus, 762 material-codec decisions per threshold: at 1e-4 e, **518/533 = 97.2%** of naive failures are QSQ non-evaluable and only 15 are genuine eligible failures; at 1e-3 e, **296/310 = 95.5%** are non-evaluable and only 14 genuine; at 1e-2 e, **61/108 = 56.5%** are non-evaluable. Eligibility also invalidates apparent successes: at 1e-4 e, **106/229 = 46.3%** of naive passes are non-evaluable | `analysis/CERTIFIABILITY_AUDIT_20260911.md`, `analysis/certifiability_reclassification_pooled_20260911.csv`, `analysis/certifiability_reclassification_by_codec_20260911.csv` | **F3** | **CONFIRMED** | This is a label-validity result, not a claim that QSQ rescues codecs. Non-evaluable is neither pass nor failure |
