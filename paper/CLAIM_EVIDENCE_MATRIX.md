...
| 17 | Strict QSQ numerical qualification is not equivalent to preservation of a coarse qualitative chemical direction | Outcome-blind P4 case study: 5/5 frozen chemistry pairs pass the two-implementation source-reference gate; 216/216 compressed solver cells resolve; all 60/60 common-tight pair-codec-setting BaderKit sign decisions preserve the reference direction. QSQ at 1e-3 e retains 36/60 trials from 3/5 pairs, while no qualification retains 60/60; both have 0 observed sign errors. Optional QSQ-targeted independent-solver audit uses 48 Henkelman state calls / 397.2 s versus 108 / 733.3 s for escalate-all. | `validation/qsq_prospective/p4_candidate_audit/`; `validation/qsq_prospective/p4_reference_adjudication/`; `validation/qsq_prospective/p4_chemical_decisions_resolved/` | S15-S16 / Discussion | **CONFIRMED** for the frozen case-study endpoint | Small 5-pair cohort; sign endpoint has a large chemical margin and is not a prevalence estimate; QSQ shows no correctness gain because the baseline is already error-free; no DFT grid-convergence claim |
