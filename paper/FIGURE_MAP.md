...
`measurement contract -> operator motivation -> binary-benchmark invalidity -> qualification-probe validation -> Bader mechanism -> fair codec comparison -> untouched external confirmation`
...
| 4 | Protocol A -> A.1 probe correction and eligibility qualification | `figures/R/figure4_stability_protocol.R` | `figures/R/rendered/figure4_stability_protocol_R.{png,pdf,svg}` | **LOCKED** |
...
### B — Protocol A.1 three-state certification
...
**Interpretation boundary:** Protocol A.1 does not “rescue” failing codecs. A non-evaluable material-threshold pair is neither pass nor failure; the binary label itself is invalid.
...
## Figure 4 — Qualification-probe validation

**Function:** validates why Protocol A.1, rather than archived Protocol A, defines eligibility.
...
External A.1 eligibility is 16/63 at `1e-4 e`, 42/63 at `1e-3 e`, and 57/63 at `1e-2 e`. Median best-certified compression ratios remain tolerance dependent: approximately ZFP/SZ3/SPERR = 13.0/12.1/5.2× at `1e-4 e`, 18.8/18.8/6.4× at `1e-3 e`, and 40.6/65.9/10.8× at `1e-2 e`.
...
Keep the following out of the main figure sequence: full electron-count and Hartree tables, reproduction-gate diagnostics, no-exclusion sensitivity, floor-relative sensitivity, Protocol A.1 seed/amplitude sensitivity, negative algorithm results, failure taxonomy, and external-baseline unit conversions.
