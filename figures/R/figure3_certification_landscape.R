...
d <- read.csv(file.path(root, "benchmark", "master_benchmark_full.csv"),
...
summarize_material <- function(g, cert_col, elig_col, ign_col) {
  g %>% group_by(material_id) %>% summarise(
    ...
    ign=any(as_flag(.data[[ign_col]]), na.rm=TRUE), .groups="drop")
}

S <- bind_rows(lapply(c("ZFP","SZ3","SPERR"), function(cc) {
  bind_rows(lapply(taus, function(t) {
    ...
    if (!all(needed %in% names(g))) stop("Missing certification columns")
    z <- summarize_material(g, cert_col, elig_col, ign_col)
    data.frame(codec=cc, tau=t, n_materials=nrow(z),
               cert=100*mean(z$cert), elig=100*mean(z$elig),
               ign=100*mean(z$ign), overstatement=100*mean(z$ign)-100*mean(z$cert))
  }))
}))
...
p1 <- ggplot(S, aes(tau, codec, fill=cert)) +
  ...
  geom_text(aes(label=sprintf("%.0f%%", cert)), fontface="bold", size=4.0, colour=ink) +
  scale_fill_gradientn(colours=c("#F4F6F5", pal[["light_green"]], pal[["green"]], pal[["navy"]]),
                       limits=c(0,100), guide="none") +
  labs(title="A | Certification depends on tolerance and codec",
       subtitle="Protocol A.1 eligible + re-derived Bader error below tau",
       x="Chemical tolerance tau (e)", y=NULL) + th + theme(panel.grid=element_blank())

# Eligibility is a material property and is codec independent; show it once.
E <- S %>% filter(codec=="ZFP")
p2 <- ggplot(E, aes(tau, elig, group=1)) +
  geom_line(linewidth=1.15, colour=pal[["navy"]]) +
  geom_point(size=3.0, colour=pal[["navy"]]) +
  geom_text(aes(label=sprintf("%.0f%%", elig)), vjust=-.7, size=3.2, colour=ink) +
  scale_y_continuous(limits=c(0,100), breaks=seq(0,100,20), labels=label_percent(scale=1)) +
  labs(title="B | The A.1 stability floor sets the identifiability ceiling",
       subtitle="Eligibility is defined before codec success is counted",
       x="Chemical tolerance tau (e)", y="Eligible materials") + th

ord <- expand.grid(codec=factor(c("ZFP","SZ3","SPERR"), levels=c("ZFP","SZ3","SPERR")),
...
p3 <- ggplot(S, aes(y=key)) +
  geom_segment(aes(x=cert, xend=ign, yend=key), colour="#C7C9CC", linewidth=1.8) +
  geom_point(aes(x=ign), shape=21, fill=bg, colour="#777C82", stroke=.85, size=3.4) +
  geom_point(aes(x=cert, colour=codec), size=3.4) +
  ...
  labs(title="C | Ignoring eligibility systematically overstates scientific success",
       subtitle="Filled: Protocol A.1 certified; open: Bader criterion counted without eligibility",
       x="Material fraction", y="codec  |  tau") + th

fig <- ((p1 | p2) / p3) +
  plot_layout(heights=c(.95,1.08), widths=c(1,1)) +
  plot_annotation(
    title="Figure 3 | Chemical certification is a stability-aware decision problem",
    subtitle="Compression fidelity is scientifically interpretable only after the downstream observable is identifiable at the requested tolerance.",
    caption="Development corpus. Protocol A.1 uses fixed-seed uniform perturbations at the float32 L-infinity amplitude. Certification requires both eligibility and re-derived Bader error below the chemical tolerance.",
