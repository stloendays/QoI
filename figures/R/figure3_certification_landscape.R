...
d <- read.csv(file.path(root, "benchmark", "master_benchmark_full.csv"), check.names = FALSE, stringsAsFactors = FALSE)
...
summarise_one_decision <- function(g, cert_col, elig_col, ign_col) {
  g %>% group_by(material_id) %>% summarise(
...
D <- bind_rows(lapply(taus, function(t) {
  bind_rows(lapply(codecs, function(cc) {
    ...
    if (!all(c(cert_col, elig_col, ign_col) %in% names(g))) stop("Missing Figure 3 certification columns for tau = ", t)
    summarise_one_decision(g, cert_col, elig_col, ign_col) %>% mutate(
      codec = cc, tau = t,
...
S <- D %>% group_by(tau) %>% summarise(
  n_decisions = n(),
  naive_pass = sum(naive_pass), naive_fail = sum(naive_fail),
  qualified_pass = sum(certified), eligible_fail = sum(eligible_fail),
...
# Frozen-data assertions: fail CI if the underlying benchmark changes silently.
expected <- data.frame(
  tau = taus,
...
for (nm in setdiff(names(expected), "tau")) {
  if (!identical(as.integer(S[[nm]]), as.integer(expected[[nm]]))) stop("Frozen Figure 3 assertion failed for column: ", nm)
}

col_grey <- "#9AA0A6"; col_red <- "#D94B41"; col_teal <- "#2A9D8F"; col_orange <- "#E9B13A"
col_text <- "#1F2937"; col_grid <- "#E5E7EB"; col_box <- "#F8FAFC"
bar_half <- 0.36
base_theme <- theme_minimal(base_size = 11.2) + theme(
  ...
  plot.title = element_text(face = "bold", colour = col_text, size = 14.2, margin = margin(b = 4)),
  plot.subtitle = element_text(colour = "#4B5563", size = 10.0, lineheight = 1.05, margin = margin(b = 8)),
  legend.title = element_blank(), legend.text = element_text(size = 9.6, colour = col_text),
  plot.margin = margin(10, 12, 8, 10)
)

p_a <- ggplot(S, aes(x = x)) +
  geom_rect(aes(xmin = x-bar_half, xmax = x+bar_half, ymin = 0, ymax = naive_pass, fill = "Naive pass"), colour = "white", linewidth = 0.55) +
  geom_rect(aes(xmin = x-bar_half, xmax = x+bar_half, ymin = naive_pass, ymax = n_decisions, fill = "Naive fail"), colour = "white", linewidth = 0.55) +
  geom_text(aes(y = naive_pass/2, label = naive_pass), colour = "white", fontface = "bold", size = 4.4) +
  geom_text(aes(y = naive_pass + naive_fail/2, label = naive_fail), colour = "white", fontface = "bold", size = 4.4) +
  geom_text(aes(y = 794, label = "Total = 762"), colour = col_text, size = 3.45) +
  ...
  labs(title = "A  Naive binary benchmark", subtitle = "762 development material-codec decisions per threshold", x = "Bader threshold", y = "Number of decisions") +
...
p_b <- ggplot(S, aes(x = x)) +
  geom_rect(aes(xmin = x-bar_half, xmax = x+bar_half, ymin = 0, ymax = qualified_pass, fill = "Certified"), colour = "white", linewidth = 0.55) +
  geom_rect(aes(xmin = x-bar_half, xmax = x+bar_half, ymin = qualified_pass, ymax = qualified_pass+eligible_fail, fill = "Eligible failure"), colour = "white", linewidth = 0.55) +
  geom_rect(aes(xmin = x-bar_half, xmax = x+bar_half, ymin = qualified_pass+eligible_fail, ymax = n_decisions, fill = "Non-evaluable"), colour = "white", linewidth = 0.55) +
  geom_text(aes(y = qualified_pass/2, label = qualified_pass), colour = "white", fontface = "bold", size = 4.4) +
  geom_text(aes(y = qualified_pass + eligible_fail/2, label = eligible_fail), colour = "white", fontface = "bold", size = 3.7) +
  geom_text(aes(y = qualified_pass + eligible_fail + non_evaluable/2, label = non_evaluable), colour = "white", fontface = "bold", size = 4.4) +
  geom_text(aes(y = 794, label = "Total = 762"), colour = col_text, size = 3.45) +
...
S_c <- S %>% mutate(callout_y = c(675, 540, 350), naive_label_y = naive_fail + c(28,27,24))
p_c <- ggplot(S_c, aes(x = x)) +
  geom_rect(aes(xmin = x-bar_half, xmax = x+bar_half, ymin = 0, ymax = genuine_eligible_failure, fill = "Genuine eligible failure"), colour = "white", linewidth = 0.55) +
  geom_rect(aes(xmin = x-bar_half, xmax = x+bar_half, ymin = genuine_eligible_failure, ymax = naive_fail, fill = "Reclassified non-evaluable"), colour = "white", linewidth = 0.55) +
  geom_text(aes(y = genuine_eligible_failure/2, label = genuine_eligible_failure), colour = "white", fontface = "bold", size = 3.7) +
  geom_text(aes(y = genuine_eligible_failure + naive_fail_reclassified_non_evaluable/2, label = naive_fail_reclassified_non_evaluable), colour = "white", fontface = "bold", size = 4.4) +
  geom_text(aes(y = naive_label_y, label = paste0("Naive failures = ", naive_fail)), colour = col_text, size = 3.35) +
  geom_label(aes(y = callout_y, label = paste0(percent(fraction_naive_fail_reclassified, accuracy = 0.1), "\nreclassified")), fill = "white", colour = col_red, fontface = "bold", label.size = 0.35, size = 4.15, lineheight = 0.92) +
...
p_key <- ggplot() + coord_cartesian(xlim = c(0,1), ylim = c(0,1), clip = "off") + theme_void() +
  ...
  annotate("rect", xmin=.005, xmax=.16, ymin=.08, ymax=.92, fill=col_box, colour="#94A3B8", linewidth=.45) +
...
fig <- ((p_a | p_b | p_c) / p_key) + plot_layout(heights = c(8.4,1.6)) +
  plot_annotation(title = "Figure 3 | Binary benchmark vs stability-qualified three-state certification",
                  theme = theme(plot.background = element_rect(fill="white", colour=NA),
                                plot.title = element_text(face="bold", size=18.5, hjust=.5, colour=col_text, margin=margin(b=7))))
...
ggsave(svg_path, fig, width=14.2, height=9.0, bg="white", device=svglite::svglite)
message("Rendered: ", png_path)
message("Rendered: ", pdf_path)
message("Rendered: ", svg_path)
message("Headline reclassification: ", paste(percent(S$fraction_naive_fail_reclassified, accuracy=0.1), collapse=" / "))
