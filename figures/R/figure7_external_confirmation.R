...
extdir <- file.path(root, "validation", "final_external_confirmatory63_20260908", "confirmatory63")
...
stopifnot(file.exists(summary_path), file.exists(pair_path), file.exists(best_path), file.exists(audit_path))
...
codec_label <- function(x) toupper(as.character(x))
...
a <- S %>% filter(stratum=="overall", threshold_e %in% tau_levels) %>%
...
pA <- ggplot(a, aes(tau, ratio_median, colour=codec, group=codec)) +
  geom_errorbar(aes(ymin=ratio_median_ci_lo, ymax=ratio_median_ci_hi), width=.13, linewidth=.7, alpha=.78) +
...
pB <- ggplot(b, aes(tau, frac_a_wins)) +
  ...
  geom_errorbar(aes(ymin=a_wins_ci_lo, ymax=a_wins_ci_hi), width=.13, linewidth=.75, colour=pal[["orange"]]) +
  geom_line(aes(group=1), linewidth=1.1, colour=pal[["orange"]]) + geom_point(size=3.0, colour=pal[["orange"]]) +
  geom_text(aes(label=percent(frac_a_wins, accuracy=1)), vjust=-.75, size=3.0, colour=ink) +
  scale_y_continuous(limits=c(0,1), breaks=seq(0,1,.25), labels=label_percent()) +
  labs(title="B | The ZFP-SZ3 ordering changes with scientific tolerance",
       subtitle="Eligible external materials where SZ3 has the higher certified ratio",
       x="Bader contract tau (e)", y="SZ3 win fraction") + th + theme(legend.position="none")

as_flag <- function(x) x %in% c(TRUE,1,"1","TRUE","True","true")
c <- B %>% filter(as_flag(has_certified_point), is.finite(realized_Linf_over_nominal_at_CCR), realized_Linf_over_nominal_at_CCR>0) %>%
  mutate(codec=factor(codec_label(codec), levels=c("ZFP","SZ3","SPERR")))
c_med <- c %>% group_by(codec) %>% summarise(med=median(realized_Linf_over_nominal_at_CCR, na.rm=TRUE), .groups="drop")
...
pC <- ggplot(c, aes(codec, realized_Linf_over_nominal_at_CCR, fill=codec, colour=codec)) +
  ...
  geom_text(data=c_med, aes(x=codec, y=med, label=sprintf("median %.2f", med)), inherit.aes=FALSE,
            vjust=-1.05, size=2.9, colour=ink) +
  geom_hline(yintercept=1, linetype=2, linewidth=.55, colour="#6B7076") +
  scale_fill_manual(values=codec_cols, guide="none") + scale_colour_manual(values=codec_cols, guide="none") +
  scale_y_log10(limits=c(.05,1.7), breaks=c(.05,.1,.2,.5,1), labels=label_number(accuracy=.01)) +
  labs(title="C | Nominal tolerance is not a common distortion scale",
       subtitle="Certified external points; dashed line marks full use of the nominal budget",
       x=NULL, y="Realized L-inf / nominal budget") + th + theme(legend.position="none")

d <- S %>% filter(stratum=="overall", codec=="zfp", threshold_e %in% tau_levels) %>%
  mutate(tau=factor(threshold_e, levels=tau_levels, labels=tau_labels), eligible=n_admitted,
         frac=eligible/A$n_materials_complete, label_y=c(21,47,62.5))

audit_text <- sprintf("%d/%d complete | %d material failures | %d bound violations\n%d retained scientific rows",
...
pD <- ggplot(d, aes(tau, eligible, group=1)) +
  geom_segment(aes(x=tau, xend=tau, y=0, yend=eligible), linewidth=4.4, colour=alpha(pal[["light_green"]], .72)) +
  geom_line(linewidth=1.0, colour=pal[["navy"]]) + geom_point(size=3.0, colour=pal[["navy"]]) +
  geom_text(aes(y=label_y, label=sprintf("%d / %d\n(%s)", eligible, A$n_materials_complete, percent(frac, accuracy=1))),
            size=3.0, colour=ink) +
